...
public class RC4 {
    ...
    public static void main (String [] args) throws NoSuchPaddingException, NoSuchAlgorithmException, InvalidKeyException, BadPaddingException, IllegalBlockSizeException, IOException, NoSuchProviderException {
        String key = "5b7c959d92e969e9";
        ...
        result = RC4method(key, ciphertext);
        asciiresult = CheckifCorrect(result);
        //if(asciiresult >= 29 && result.length() == 100)
    ...
    private static String RC4method(String strbyte, String cryptog) throws NoSuchPaddingException, NoSuchAlgorithmException, InvalidKeyException, BadPaddingException, IllegalBlockSizeException, IOException, NoSuchProviderException {
        Cipher cipher = Cipher.getInstance("RC4");
        System.out.println(strbyte.getBytes(StandardCharsets.US_ASCII).toString());
        SecretKeySpec rc4key = new SecretKeySpec(strbyte.getBytes("ASCII"), "RC4");
        System.out.println(rc4key);

        cipher.init(Cipher.DECRYPT_MODE, rc4key);
        byte[] dec = new sun.misc.BASE64Decoder().decodeBuffer(cryptog);
        //byte[] cleartext = cipher.doFinal(dec);
        byte[] cleartext = cipher.update(cryptog.getBytes("ASCII"));
